...
func TestCookieDomainForContext(t *testing.T) {
	t.Run("selects most specific configured domain", func(t *testing.T) {
		t.Cleanup(config.SetTestConfig(func(c *config.Config) {
			...
			c.Server.Domains = []config.ConfigServerDomain{
				...
				{Domain: "apps.example.com", AuthHost: "auth.apps.example.com"},
			}
		}))
		...
		domain, authHost, ok := cookieDomainForContext(c)
		require.True(t, ok)
		assert.Equal(t, "apps.example.com", domain)
		assert.Equal(t, "auth.apps.example.com", authHost)
	})
	...
	t.Run("uses request host when domains are empty", func(t *testing.T) {
		...
		assert.Equal(t, "app.example.org", authHost)
	})

	t.Run("rejects unmatched configured domains", func(t *testing.T) {
		t.Cleanup(config.SetTestConfig(func(c *config.Config) {
			c.Cookies.Domain = ""
			c.Server.Domains = []config.ConfigServerDomain{{Domain: "example.com", AuthHost: "example.com"}}
		}))
		...
		_, _, ok := cookieDomainForContext(c)
		assert.False(t, ok)
	})
}
