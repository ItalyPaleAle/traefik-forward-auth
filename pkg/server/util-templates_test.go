...
func TestSetPagesPortalConfig(t *testing.T) {
	t.Run("both backgrounds empty - use defaults", func(t *testing.T) {
		...
		const expectedHeader = `default-src 'none'; script-src 'nonce-NONCE'; style-src 'self' 'nonce-NONCE'; img-src 'self'; font-src 'self'; frame-ancestors 'none'; form-action 'self'`
	...
	t.Run("custom large background only", func(t *testing.T) {
		...
		expectedHeader := `default-src 'none'; script-src 'nonce-NONCE'; style-src 'self' 'nonce-NONCE'; img-src 'self' https://cdn.example.com; font-src 'self'; frame-ancestors 'none'; form-action 'self'`
		assert.Equal(t, expectedHeader, portal.PagesCSPHeader("NONCE"))
	})
	...
	t.Run("custom medium background only", func(t *testing.T) {
		...
		assert.Equal(t, defaultPagesBackgroundLarge, portal.PagesBackgroundLarge)
		expectedHeader := `default-src 'none'; script-src 'nonce-NONCE'; style-src 'self' 'nonce-NONCE'; img-src 'self' https://images.example.com; font-src 'self'; frame-ancestors 'none'; form-action 'self'`
		assert.Equal(t, expectedHeader, portal.PagesCSPHeader("NONCE"))
	})
	...
	t.Run("both backgrounds from different origins", func(t *testing.T) {
		...
		expectedHeader := `default-src 'none'; script-src 'nonce-NONCE'; style-src 'self' 'nonce-NONCE'; img-src 'self' https://cdn1.example.com https://cdn2.example.com; font-src 'self'; frame-ancestors 'none'; form-action 'self'`
		assert.Equal(t, expectedHeader, portal.PagesCSPHeader("NONCE"))
	})
	...
	t.Run("HTTP backgrounds from different origins", func(t *testing.T) {
		...
		expectedHeader := `default-src 'none'; script-src 'nonce-NONCE'; style-src 'self' 'nonce-NONCE'; img-src 'self' cdn1.example.com cdn2.example.com; font-src 'self'; frame-ancestors 'none'; form-action 'self'`
		assert.Equal(t, expectedHeader, portal.PagesCSPHeader("NONCE"))
	})
	...
	t.Run("mixed HTTP and HTTPS backgrounds", func(t *testing.T) {
		...
		assert.Equal(t, "http://cdn2.example.com/medium.jpg", portal.PagesBackgroundMedium)
		assert.Equal(t, "https://cdn1.example.com/large.jpg", portal.PagesBackgroundLarge)
		expectedHeader := `default-src 'none'; script-src 'nonce-NONCE'; style-src 'self' 'nonce-NONCE'; img-src 'self' https://cdn1.example.com cdn2.example.com; font-src 'self'; frame-ancestors 'none'; form-action 'self'`
		assert.Equal(t, expectedHeader, portal.PagesCSPHeader("NONCE"))
	})
	...
	t.Run("backgrounds with ports", func(t *testing.T) {
		...
		expectedHeader := `default-src 'none'; script-src 'nonce-NONCE'; style-src 'self' 'nonce-NONCE'; img-src 'self' https://cdn.example.com:8443 https://cdn.example.com:9443; font-src 'self'; frame-ancestors 'none'; form-action 'self'`
		assert.Equal(t, expectedHeader, portal.PagesCSPHeader("NONCE"))
	})
	...
	t.Run("relative URL backgrounds", func(t *testing.T) {
		...
		expectedHeader := `default-src 'none'; script-src 'nonce-NONCE'; style-src 'self' 'nonce-NONCE'; img-src 'self'; font-src 'self'; frame-ancestors 'none'; form-action 'self'`
		assert.Equal(t, expectedHeader, portal.PagesCSPHeader("NONCE"))
	})
	...
	t.Run("backgrounds with query parameters", func(t *testing.T) {
		...
		const expectedHeader = `default-src 'none'; script-src 'nonce-NONCE'; style-src 'self' 'nonce-NONCE'; img-src 'self' https://cdn.example.com; font-src 'self'; frame-ancestors 'none'; form-action 'self'`
		assert.Equal(t, expectedHeader, portal.PagesCSPHeader("NONCE"))
	...
	t.Run("CSP header format check", func(t *testing.T) {
		...
		assert.Contains(t, cspHeader, "style-src 'self' 'nonce-NONCE'")
