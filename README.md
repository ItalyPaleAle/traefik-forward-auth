...
| <a id="config-opt-metricsserverenabled"></a>YAML: `metricsServerEnabled`<br>Env: `TFA_METRICSSERVERENABLED` | boolean | Enable the metrics server, which exposes a Prometheus-compatible endpoint `/metrics`.<br>Metrics must be enabled for this to be effective| Default: _false_ |
