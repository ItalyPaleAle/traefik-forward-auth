...
| <a id="config-opt-cookies-domains"></a>`cookies.domains` | list of strings | Domain names for setting cookies<br>If set, the cookie domain is selected for each request by matching the request hostname against this list<br>This can be used when one Traefik Forward Auth instance serves multiple unrelated domains| Recommended |
